...
use crate::config::{ConfigKey, ConfigKeyValue, ConfigMap};
...
impl ConfigRepository {
    ...
    pub fn find_by_key(&self, key: ConfigKey) -> Result<Option<ConfigKeyValue>, DbError> {
        ...
        let entity = entity.map(|e| e.try_into().ok()).flatten();
...
mod tests {
    ...
    fn clean(connection: &mut SqliteConnection) {
        let _ = diesel::delete(config_table::table).execute(connection);
    }
}

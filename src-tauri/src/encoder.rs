...
impl std::fmt::Display for ImageEncodeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ImageEncodeError::NotImageError { message } => {
                write!(f, "not image error: {}", message)
            }
            ImageEncodeError::DetectTypeError { message } => {
                write!(f, "detect type error: {}", message)
            }
        }
    }
}
...
pub fn encode_image_base64(path: PathBuf) -> Result<String, ImageEncodeError> {
    let mime_type = detect_mime_type(path.clone()).map_err(|err| {
        ...
        ImageEncodeError::DetectTypeError {
    ...
    let bytes = std::fs::read(path.clone()).map_err(|err| ImageEncodeError::ReadError {
        message: err.to_string(),
    })?;

...
impl ServerRepository {
    ...
    pub fn find_by_id(&self, id: Uuid) -> Result<Option<Server>, DbError> {
        ...
        let entity = entity.map(|entity| entity.try_into().ok()).unwrap_or(None);
...
mod tests {
    ...
    fn server_repository_find_by_url_should_return_empty_when_not_found() {
        ...
        let actual = repository
            .find_by_url("https://nonexistent.com".to_string())
            .unwrap();
    ...
    fn clean(connection: &mut SqliteConnection) {
        let _ = diesel::delete(servers_table::table).execute(connection);
    }
}
